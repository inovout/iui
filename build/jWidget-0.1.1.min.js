...
Inovout.HAML.EncryptInput=Class.create({initialize:function(b){var a=b.attr("name"),c=b.data("encrypt").split("|");b.removeAttr("name");var d=$("<input type='hidden' name="+a+" />");b.append(d);b.change.addListener(function(a,k){var f=b.val(),l=new RSAKey;l.setPublic(c[0],c[1]);d.val(l.encrypt(f))});return this}});
Inovout.HAML.Parsers.EncryptInputParser={parse:function(b){b.find("[data-encrypt]").each(function(a){var b=a.attr("name");new Inovout.HAML.EncryptInput(a);""!=a.val()&&""==$("input[name='"+b+"']").val()&&a.trigger("change")})}};Inovout.Widgets={};Inovout.View=Class.create({initialize:function(b){this.element=Inovout.Element.get(b);return this}});
