...
Inovout.HAML.EncryptInput=Class.create({initialize:function(b){var a=b.attr("name"),c=b.data("encrypt").split("|");b.removeAttr("name");var d=$("<input type='hidden' name="+a+" />");b.after(d);Inovout.Element.get(b.parents("form")).submit.addListener(function(a,l){var f=b.val(),k=new RSAKey;k.setPublic(c[0],c[1]);d.val(k.encrypt(f))});return this}});Inovout.HAML.Parsers.EncryptInputParser={parse:function(b){b.find("[data-encrypt]").each(function(a){new Inovout.HAML.EncryptInput(a)})}};
