...
Inovout.HAML.EncryptInput=Class.create({initialize:function(b){var a=b.attr("name"),c=b.data("encrypt").split("|");b.removeAttr("name");var d=$("<input type='hidden' name="+a+" />");b.append(d);b.change.addListener(function(a,k){var f=b.val(),l=new RSAKey;l.setPublic(c[0],c[1]);d.val(l.encrypt(f))});return this}});Inovout.HAML.Parsers.EncryptInputParser={parse:function(b){b.find("[data-encrypt]").each(function(b){new Inovout.HAML.EncryptInput(b)})}};Inovout.Widgets={};
...
Inovout.Widgets.DropDownList=Class.create(Inovout.Widgets.List,{initialize:function($super,a){$super(a);var c=this;a.change.addListener(function(a,e){var k=c.element.children("option:selected")[0];c.selectedChanged.fire(c,c.wrapEventArgs(k))});return this},add:function(b){this.element.append('<option value="'+b.value+'">'+b.value+"</option>")}});
...
Inovout.Widgets.Form=Class.create(Inovout.View,{initialize:function($super,a){function c(c){var f=new Uri(a.prop("action")),e=a.serializeJSON(),g=new HttpRequest(a.prop("method"),f),h,m=new HttpClient;enctype=a.attr("enctype");if(0<f.uri.indexOf("["))for(name in e)f.uri=f.uri.replace(RegExp("\\["+name+"\\]","ig"),e[name]);"post"==g.method&&("application/x-www-form-urlencoded"==enctype?h=new JsonContent(e):"multipart/related"==enctype&&(f="iui_"+(""+Math.random()).replace(/\D/g,""),h=new MultipartContent("related",
f),h.add(new JsonContent(e)),a.find(".fileInput").each(function(a){h.add(new FileContent(Inovout.View.get(a.fileInput.files[0])))})),g.content=h);m.send(g).read().done(function(a){Inovout.View.buildFunction("data",c).call(d,a)})}$super(a);var d=this,e=a.data("async");e&&(a.complete(a.submit,function(a){a.defaultPrevented||(a.preventDefault(),a.stopPropagation(),c(e))}),a.find("input[type=file]").each(function(a){var c=$('<div class="fileInput"><button type="button" class="fileInput_browse_button">\u6d4f\u89c8</button></div>')[0];
...
Inovout.Widgets.DataTable=Class.create(Inovout.View,{initialize:function($super,a){$super(a);this.selectedRowsChanged=new Event("selectedRowsChanged",this);return this},selectRow:function(b){var a=b.parent().parent();this.element.find(".selected").each(function(a){a.removeClass("selected")});a.toggleClass("selected");this.selectedRowsChanged.fire(this,this.wrapEventArgs(b))},insertNewRow:function(b){b=$.templates("#tr_tmplate").render(b);b=new Inovout.Element("<tr>"+b+"</tr>");this.element.append(b);
Inovout.HAML.Parser.parse(this.element.find("tr:last")[0])},wrapEventArgs:function(b){b=!1;var a=0;this.element.find(".selected").each(function(b){a++});0<a&&(b=!0);return{hasSelected:b}}});
Inovout.Widgets.Dialog=Class.create(Inovout.View,{initialize:function(b,a,c){this.widgetDialog=$('<iframe title="\u5185\u5bb9\u9875"  src='+b+"></iframe>").dialog({autoOpen:!0,model:!0,height:c,width:a});return this},messageHandle:function(b){execueDone(b.data)},addEventLister:function(b,a){var c=this.dom;c.addEventListener?c.addEventListener(b,a,!1):c.attachEvent&&c.attachEvent("on"+b,a)},removeEventtLister:function(b,a){var c=this.dom;if(c.removeEventListener)c.removeEventListener(b,a,!1);else if(c.detachEvent){var d=
"on"+b;typeof c[d]===core_strundefined&&(c[d]=null);c.detachEvent(d,a)}},done:function(b){this.fnexpression=b},execueDone:function(b){Inovout.View.buildFunction("me",this.fnexpression).call(this,b);this.widgetDialog.dialog("close")},close:function(){this.widgetDialog.dialog("close")},receiveMess:function(b){this.execueDone(b)}});
